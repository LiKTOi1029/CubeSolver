...
local BooleanTree = 
{
  [false] = 
  {
	...
	[false] = function() -- Normal
	  
	end,
  },
  [true] = 
  {
	[false] = function() -- Reversal
	  
	end,
	[true] = function() -- Reverse and DoubleMove
...
function Moveset.Execute(Input, Cube)
  ...
  if Moveset[Input] then
	...
	if Reversal then BooleanTable[1] == true end
	if DoubleMove then BooleanTable[2] == true end
